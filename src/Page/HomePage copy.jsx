...
function HomePage() {
  ...
  const añadirAlCarrito = (producto) => {
    const productoExistente = itemCarrito.find((p) => p._id === producto._id);
            ...
            {categoryAll.length > 0
              ? categoryAll.map((category, index) => (
                  <li
                    ...
                    key={index}
                  >
                    <div className="bg-[#ffffff] bg-[#0000000e] h-[70vh] group relative group/foto">
                      {/* Juego de imagenes - Max 2 img */}
                      <img
                        src={category.url}
                        alt="producto"
                      ...
                      <img
                        ...
                        loading="lazy"
                      />
                      <div className="absolute bottom-1 px-3 py-2 m-5 text-[25px] font-extrabold disabled">
                        <p className="-mb-3">{category.category}</p>
                        <button className="text-sm md:font-medium border-b border-[#000]">
                          VER MÁS
                        </button>
                      </div>
                    </div>
